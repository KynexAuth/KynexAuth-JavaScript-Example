...
class KynexAuth {
    ...
    constructor({ name, ownerid, ownerId, version = '1.0', url, apiUrl, debug = false }) {
        ...
        this._hwidCache = '';
        this._ipCache = '';
        this.get_public_ip().then(ip => { this._ipCache = ip; }).catch(() => {});
    }
    ...
    async get_public_ip() {
        ...
        for (const svcUrl of services) {
            try {
                const ip = await new Promise((resolve, reject) => {
                    const req = https.get(svcUrl, { timeout: 3000 }, (res) => {
                        let data = '';
    ...
    _parseLoginResponse(resStr, defaultUsername = '', caller = 'login') {
        try {
            ...
            if (this.response.success) {
                ...
                let userIp = String(info.ip || data.ip || this._ipCache || '127.0.0.1');
